...
int main(int argc, char** argv) {
    ...
    if (way == 1) { // Bresenham's line algorithm
        ...
        image.DrawLine(x1, y1, x2, y2, white); // Алгоритм Брезенхема для отрезков
        image.set(x1, y1, red);
        image.set(x2, y2, red);
    }
    ...
    if (way == 2) { // simple DDA line algorithm
        ...
        image.DrawLineDDA(x1, y1, x2, y2, white); // Алгоритм Брезенхема для отрезков
        image.set(x1, y1, red);
        image.set(x2, y2, red);
    ...
    if (way == 5) { // Bresenham's circle algorithm
       ...
       int x0, y0, R;
       std::cin >> x0 >> y0 >> R;
       image.CircleBres(x0,y0, R, white);
       image.set(x0, y0, red);
    }
